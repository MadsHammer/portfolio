...
const ProjectList = () => {
  ...
  if (isLoading)
    return (
      <p className="text-center text-green-500 py-20">Loading Projects...</p>
    );
  if (isError)
    return (
      <p className="text-center text-red-500 py-20">Error loading data.</p>
    );
      ...
      <div className="max-w-6xl custom-max-width mx-auto px-6 py-20">
        <div className="flex flex-col gap-32">
          {projects?.map((project: Project, index: number) => (
            <div
              key={project.id}
              className="flex flex-col  border-b border-gray-200 pb-20"
            >
              ...
              <div className="flex flex-col lg:flex-row gap-12 items-center">
                <div className="w-full lg:w-4/12">
                  ...
                  <h5 className="text-4xl lg:text-5xl font-bold text-gray-900 mb-6 ">
                    {project.title}
                  </h5>
                  ...
                  <div className="flex flex-wrap gap-3 mb-8">
                    ...
                    {project.techStack.map((tech) => (
                      <span
                        key={tech}
                        className="text-sm font-semibold text-green-700 bg-green-50 px-3 py-1 rounded-full border border-green-100"
                      >
                        {tech}
                      </span>
                    ))}
